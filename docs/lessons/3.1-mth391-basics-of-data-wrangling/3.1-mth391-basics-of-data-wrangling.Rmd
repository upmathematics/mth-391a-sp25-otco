...
::: {style="color: red"}
$\star$ Notice that the first line is the data frame itself, then the next line is the verb without without putting it directly into the first argument of the `filter()` verb. This is a common practice of organizing verbs in a pipeline.
:::
...
5. Implement the verbs in code line-by-line, then make adjustments.

## In-Class Demonstration

**Case Example:** Using the `iris` data set, we want to know how many rows (or observations) for each species that have length ratio greater than $0.80$ and width ratio greater than $0.5$. Here, the ratio is defined as petal over sepal variables.

:::: {.column width=49%}
**The original number of rows of each species is shown below.**
...
::: {style="color: blue"}
$\dagger$ The task here is to replicate the shown data frames using the `dplyr` verbs.
:::
